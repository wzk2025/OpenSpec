...
describe('PowerShellInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return path relative to profile directory', () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.config', 'powershell', 'RdCompletion.ps1'));
    });

    it('should work with custom PROFILE environment variable', () => {
      process.env.PROFILE = path.join(testHomeDir, 'custom', 'profile.ps1');
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'custom', 'RdCompletion.ps1'));
    });
    ...
    it('should return Windows path when on Windows platform', () => {
      ...
      const result = installer.getInstallationPath();
      expect(result).toBe(path.join(testHomeDir, 'Documents', 'PowerShell', 'RdCompletion.ps1'));
    });
  });
  ...
  describe('configureProfile', () => {
    const mockScriptPath = '/path/to/RdCompletion.ps1';
    ...
    it('should create profile with markers when file does not exist', async () => {
      ...
      expect(content).toContain('# RDSPEC:START');
      expect(content).toContain('# RDSPEC:END');
    ...
    it.skipIf(process.platform === 'win32')('should skip configuration when script line already exists', async () => {
      ...
      const initialContent = [
        '# RDSPEC:START - RdSpec completion (managed block, do not edit manually)',
        `. "${mockScriptPath}"`,
        '# RDSPEC:END',
        '',
        '# My custom config',
  ...
  describe('removeProfileConfig', () => {
    ...
    it('should remove content between markers', async () => {
      ...
      expect(result).toBe(true);
      const content = await fs.readFile(profilePath, 'utf-8');
      expect(content).not.toContain('# RDSPEC:START');
      expect(content).not.toContain('# RDSPEC:END');
  ...
  describe('install', () => {
    const mockCompletionScript = `# PowerShell completion script for RdSpec
$rdCompleter = {
    param($wordToComplete, $commandAst, $cursorPosition)
    # Completion logic here
}
Register-ArgumentCompleter -CommandName rd -ScriptBlock $rdCompleter
`;
    ...
    it('should install completion script for the first time', async () => {
      ...
      expect(result.installedPath).toContain('RdCompletion.ps1');
  ...
  describe('encoding preservation', () => {
    ...
    it('should preserve UTF-16 LE BOM when removing profile config', async () => {
      ...
      // Verify content: original line kept, RdSpec block removed
      const content = raw.subarray(2).toString('utf16le');
    ...
    it('should round-trip UTF-16 LE through install → uninstall without corruption', async () => {
      ...
      // Uninstall removes the RdSpec block
...
`;
    ...
    it('should handle both script and config removal', async () => {
      ...
      expect(profileContent).toContain('# RDSPEC:START');
      ...
      expect(profileContentAfter).not.toContain('# RDSPEC:START');
    });
