...
describe('BashInstaller', () => {
  ...
  describe('getInstallationPath', () => {
    it('should return standard bash-completion path', async () => {
      ...
      expect(result).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'rd'));
    });
  });
  ...
  describe('install', () => {
    const testScript = '# Bash completion script for RdSpec CLI\n_rd_completion() {\n  echo "test"\n}\n';

    it('should install to bash-completion path', async () => {
      const result = await installer.install(testScript);

      expect(result.success).toBe(true);
      expect(result.installedPath).toBe(path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'rd'));
    ...
    it('should backup existing file before overwriting', async () => {
      const targetPath = path.join(testHomeDir, '.local', 'share', 'bash-completion', 'completions', 'rd');
  ...
  describe('removeBashrcConfig', () => {
    ...
    it('should remove markers and config when present', async () => {
      ...
      const content = [
        ...
        '  . ~/.local/share/bash-completion/completions/rd',
        'fi',
        '# OPENSPEC:END',
        '',
        'alias ll="ls -la"',
      ...
      expect(newContent).not.toContain('RdSpec shell completions configuration');
