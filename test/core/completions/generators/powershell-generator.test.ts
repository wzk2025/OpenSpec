...
describe('PowerShellGenerator', () => {
	...
	describe('generate', () => {
		it('should generate valid PowerShell completion script with header', () => {
			...
			expect(script).toContain('# PowerShell completion script for RdSpec CLI');
			expect(script).toContain('$rdCompleter = {');
			expect(script).toContain('Register-ArgumentCompleter');
		});

		it('should register argument completer for rd command', () => {
			const commands: CommandDefinition[] = [
				{
					name: 'init',
					description: 'Initialize RdSpec',
			...
			expect(script).toContain('Register-ArgumentCompleter -CommandName rd');
			expect(script).toContain('-ScriptBlock $rdCompleter');
		});
		...
		it('should handle positional arguments for change-id', () => {
			...
			expect(script).toContain('Get-RdChanges');
		});
		...
		it('should handle positional arguments for spec-id', () => {
			...
			expect(script).toContain('Get-RdSpecs');
		});
		...
		it('should generate dynamic completion helper for changes', () => {
			...
			expect(script).toContain('function Get-RdChanges');
			expect(script).toContain('rd __complete changes 2>$null');
		...
		it('should generate dynamic completion helper for specs', () => {
			...
			expect(script).toContain('function Get-RdSpecs');
			expect(script).toContain('rd __complete specs 2>$null');
		});
