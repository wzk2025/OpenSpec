...
function getWelcomeText(): string[] {
  return [
    chalk.white.bold('Welcome to RdSpec'),
    chalk.dim('A lightweight spec-driven framework'),
    '',
    chalk.white('This setup will configure:'),
    chalk.dim('  • Agent Skills for AI tools'),
    chalk.dim('  • /rd:* slash commands'),
    '',
    chalk.white('Quick start after setup:'),
    `  ${chalk.yellow('/rd:new')}      ${chalk.dim('Create a change')}`,
    `  ${chalk.yellow('/rd:continue')} ${chalk.dim('Next artifact')}`,
    `  ${chalk.yellow('/rd:apply')}    ${chalk.dim('Implement tasks')}`,
    '',
