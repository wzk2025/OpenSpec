...
/**
 * Read and parse .harness/spec/config.yaml from project root.
...
export function readProjectConfig(projectRoot: string): ProjectConfig | null {
  ...
  try {
    ...
    if (!raw || typeof raw !== 'object') {
      console.warn(`.harness/spec/config.yaml is not a valid YAML object`);
      return null;
    }
    ...
    console.warn(`Failed to parse .harness/spec/config.yaml:`, error);
    return null;
  }
}
...
export function suggestSchemas(
  ...
  let message = `Schema '${invalidSchemaName}' not found in .harness/spec/config.yaml\n\n`;
  ...
  message += `\nFix: Edit .harness/spec/config.yaml and change 'schema: ${invalidSchemaName}' to a valid schema name`;
