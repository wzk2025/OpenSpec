...
# Use rd __complete to get available changes
_rd_complete_changes() {
  local -a changes
  while IFS=$'\\t' read -r id desc; do
    changes+=("$id:$desc")
  done < <(rd __complete changes 2>/dev/null)
  _describe "change" changes
}

# Use rd __complete to get available specs
_rd_complete_specs() {
  local -a specs
  while IFS=$'\\t' read -r id desc; do
    specs+=("$id:$desc")
  done < <(rd __complete specs 2>/dev/null)
